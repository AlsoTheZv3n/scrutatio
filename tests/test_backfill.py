...
from scrutatio.pipeline.backfill import _chunks, _parse_watermark
...
class TestWatermarkParsing:
    def test_parses_iso_date(self) -> None:
        assert _parse_watermark("2026-08-14") == date(2026, 8, 14)

    def test_none_stays_none(self) -> None:
        assert _parse_watermark(None) is None

    def test_garbage_degrades_to_none(self) -> None:
        # A bad watermark must not abort the run; a full pass is still correct.
        assert _parse_watermark("not-a-date") is None


class TestRunBackfill:
    def test_writes_in_batches(
        self, sql: MagicMock, ctgov: MagicMock, patched: dict[str, MagicMock]
    ) -> None:
        result = run_backfill(run_token="t0", sql=sql, ctgov=ctgov, batch_size=2)
    ...
    def test_batch_names_are_unique_and_ordered(
        ...
        run_backfill(run_token="t0", sql=sql, ctgov=ctgov, batch_size=2)
    ...
    def test_batch_names_differ_between_runs(
        ...
        run_backfill(run_token="aaa", sql=sql, ctgov=ctgov, batch_size=10)
        ...
        run_backfill(run_token="bbb", sql=sql, ctgov=ctgov, batch_size=10)
...
class TestRunCompletion:
    ...
    def test_full_pass_is_marked_complete(
        ...
        result = run_backfill(run_token="t0", sql=sql, ctgov=ctgov)
    ...
    def test_limited_run_is_never_complete(
        ...
        result = run_backfill(run_token="t0", sql=sql, ctgov=ctgov, limit=3)
    ...
    def test_failure_mid_run_still_records_an_incomplete_run(
        self, sql: MagicMock, patched: dict[str, MagicMock]
    ) -> None:
        ctgov = MagicMock()
        ...
        with pytest.raises(RuntimeError, match="warehouse died"):
            run_backfill(run_token="t0", sql=sql, ctgov=ctgov)
    ...
    def test_reports_net_new_separately_from_written(
        ...
        result = run_backfill(run_token="t0", sql=sql, ctgov=ctgov, batch_size=10)
    ...
    def test_storage_is_ensured_first(
        ...
        run_backfill(run_token="t0", sql=sql, ctgov=ctgov)
        patched["ensure_storage"].assert_called_once_with(sql)
    ...
    def test_incremental_starts_from_the_bronze_watermark(
        ...
        patched["safe_watermark"].return_value = "2026-08-01"

        result = run_backfill(run_token="t0", sql=sql, ctgov=ctgov, incremental=True)
    ...
    def test_limit_is_passed_through(
        ...
        run_backfill(run_token="t0", sql=sql, ctgov=ctgov, limit=7)
        assert ctgov.iter_studies.call_args.kwargs["limit"] == 7

    def test_empty_scope_writes_nothing(
        self, sql: MagicMock, patched: dict[str, MagicMock]
    ) -> None:
        ctgov = MagicMock()
...
class TestCli:
    def test_status_prints_table_and_count(
        ...
        monkeypatch.setattr("scrutatio.pipeline.cli.SqlClient", MagicMock())
